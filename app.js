...
(function () {
  ...
  function Countdown() {
   ...
   useEffect(function() {
  console.log('[WC2026] App mounted — calling fetchMatches');
  setLod(true);
  fetchMatches();
}, []);
...
function AdSlot() {
  useEffect(function() {
    try {
      // Monetag In-Page Push renders automatically
  ...
  console.log('[WC2026] fetchMatches called');

  fetch('/api/matches')
    .then(function(r) {
      console.log('[WC2026] Response status:', r.status);
      return r.json();
    })
    .then(function(data) {
      console.log('[WC2026] Data received:', data);
      var list = [];
      if (data && data.matches) {
        list = data.matches;
      }
      console.log('[WC2026] Matches count:', list.length);
    ...
    .catch(function(err) {
      console.error('[WC2026] fetch failed:', err);
      setErr(true);
      setLod(false);
    });
}
