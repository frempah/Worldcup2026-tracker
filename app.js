...
// Built by FrempahBrand · world26cup.netlify.app
// ============================================================
...
const CACHE_TIME_KEY  = 'wc2026_cache_time_v2';
...
function initPaystack(email, onSuccess, onClose) {
  if (typeof PaystackPop === 'undefined') {
    alert('Payment unavailable. Check your internet connection and try again.');
    return;
  }
  const handler = PaystackPop.setup({
    ...
    currency: 'GHS',
    ref: 'WC2026_' + Date.now() + '_' + Math.floor(Math.random() * 99999),
    metadata: { custom_fields: [{ display_name: 'Product', value: 'WC2026 Premium' }] },
    onSuccess: (t) => { unlockPremium(); onSuccess(t); },
...
const FLAGS = {
  ...
  // Extras for Predict tab
  'TBD':'🏳️','Group A Win':'🏆','Group B 2nd':'🥈',
  'Group C Win':'🏆','Group D 2nd':'🥈',
};
...
const SLOT_LABELS = {
  ...
  'T1':'Best 3rd #1','T2':'Best 3rd #2','T3':'Best 3rd #3','T4':'Best 3rd #4',
  'T5':'Best 3rd #5','T6':'Best 3rd #6','T7':'Best 3rd #7','T8':'Best 3rd #8',
  'TBD':'To Be Determined',
};

const PREDICT_MATCHES = [
 
  {id:'p1', round:'Group Stage',   home:'Mexico',    away:'South Africa', date:'Jun 11'},
  {id:'p2', round:'Group Stage',   home:'Argentina', away:'Algeria',      date:'Jun 12'},
  {id:'p3', round:'Group Stage',   home:'Brazil',    away:'Morocco',      date:'Jun 13'},
  {id:'p4', round:'Group Stage',   home:'France',    away:'Senegal',      date:'Jun 14'},
  {id:'p5', round:'Group Stage',   home:'Spain',     away:'Uruguay',      date:'Jun 15'},
  {id:'p6', round:'Group Stage',   home:'England',   away:'Croatia',      date:'Jun 17'},
  {id:'p7', round:'Group Stage',   home:'Germany',   away:'Ecuador',      date:'Jun 16'},
  {id:'p8', round:'Group Stage',   home:'Ghana',     away:'Panama',       date:'Jun 17'},
  {id:'p9', round:'Group Stage',   home:'Portugal',  away:'Colombia',     date:'Jun 18'},
  {id:'p10',round:'Group Stage',   home:'USA',       away:'Paraguay',     date:'Jun 12'},
...
class ErrorBoundary extends React.Component {
  ...
  render() {
    ...
    return React.createElement('div', { className:'error-screen' },
      ...
      React.createElement('h2', { className:'error-title' }, 'SOMETHING WENT WRONG'),
      React.createElement('p',  { className:'error-sub'  }, 'The app hit an unexpected error'),
...
function InstallBanner() {
  ...
  async function handleInstall() {
    if (deferredPrompt) {
      ...
      if (outcome === 'accepted') { setShow(false); deferredPrompt = null; }
    } else {
  ...
  return React.createElement('div', { className:'install-banner' },
    React.createElement('div', { className:'install-left' },
      ...
      React.createElement('div', null,
        ...
        React.createElement('p', { className:'install-sub'  }, 'Free · Works offline · No App Store needed')
      )
    ),
    React.createElement('div', { className:'install-btns' },
      ...
      React.createElement('button', { className:'install-dismiss', onClick: dismiss       }, '✕')
    )
  );
}
...
function PremiumBanner({ onUnlock }) {
  ...
  if (!showForm) return React.createElement('div', { className:'premium-banner' },
    React.createElement('div', { className:'premium-left' },
      ...
      React.createElement('div', null,
        ...
        React.createElement('p', { className:'premium-sub'  }, 'Ad-free · Cloud predictions · GHS 20 once')
      )
    ),
    React.createElement('button', { className:'premium-btn', onClick:() => setShowForm(true) }, 'Unlock')
  );
  ...
  return React.createElement('div', { className:'premium-form-wrap' },
    ...
    React.createElement('input', {
      className:'premium-input', type:'email', placeholder:'Your email address',
      value:email, onChange:(e) => setEmail(e.target.value)
    }),
    React.createElement('div', { className:'premium-form-btns' },
      React.createElement('button', {
        className:'premium-pay-btn', onClick:handlePay, disabled:loading
      }, loading ? '⏳ Opening...' : '💳 Pay GHS 20'),
      React.createElement('button', {
        className:'premium-cancel-btn', onClick:() => setShowForm(false)
      }, 'Cancel')
    ),
...
function AdSlot() {
  return React.createElement('div', { className:'ad-slot' },
    ...
    // After AdSense approval, add your <ins> tag here
  );
}
...
function App() {
  ...
  async function fetchMatches() {
    try {
      ...
      setApiError(true);
      // Load stale cache rather than showing nothing
      const cached = localStorage.getItem(CACHE_KEY);
      if (cached) { try { setMatches(JSON.parse(cached)); } catch {} }
    } finally {
      setLoading(false);
    }
  }
  ...
  return React.createElement('div', { className:'app' },
    ...
    !premium && React.createElement(PremiumBanner, { onUnlock:() => setPremium(true) }),
    ...
    React.createElement('footer', { className:'app-footer' },
      ...
      React.createElement('a', { href:'/privacy.html' }, 'Privacy Policy'),
      React.createElement('span', null, '·'),
...
function MatchCard({ match }) {
  ...
  const date      = new Date(match.utcDate).toLocaleDateString('en-GB', { day:'numeric', month:'short', year:'numeric' });
    ...
    React.createElement('table', { className:'group-table' },
      ...
      React.createElement('tbody', null,
        group.teams.map((team, i) =>
          React.createElement('tr', { key:team, className: i < 2 ? 'qualify-row' : '' },
    ...
    React.createElement('div', { className:'bracket-matches' },
      matches.map((match, idx) =>
        React.createElement(BracketCard, {
          key: match.id, match, matchNum: idx + 1,
...
function BracketCard({ match, matchNum, isFinal, isSelected, onSelect }) {
  ...
  return React.createElement('div', {
    ...
    React.createElement('div', { className:'bracket-match-num' }, isFinal ? '🏆 FINAL' : `Match ${matchNum}`),
    React.createElement('div', { className:'bracket-teams' },
      ['home','away'].map(side =>
        React.createElement('div', { key:side, className:'bracket-team' },
          React.createElement('div', { className:'bracket-team-left' },
            React.createElement('span', { className:'bracket-flag' }, FLAGS[match[side]] || '🏳️'),
            React.createElement('div', { className:'bracket-team-info' },
              React.createElement('span', { className:'bracket-team-name' }, match[side]),
              React.createElement('span', { className:'bracket-team-sub'  }, side === 'home' ? homeLabel : awayLabel)
            )
          ),
          React.createElement('span', { className:'bracket-score' }, isPending ? '-' : '0'),
          side === 'home' && React.createElement('div', { className:'bracket-divider' },
    ...
    isSelected && React.createElement('div', { className:'bracket-detail' },
      React.createElement('span', null,
        isPending ? '⏳ Teams confirmed after Group Stage' : '📍 Venue & date to be announced'
      )
    )
  );
}
...
function PredictTab() {
  const [predictions, setPredictions] = useState(() => {
    try { return JSON.parse(localStorage.getItem('wc2026_predictions') || '{}'); } catch { return {}; }
  });
  ...
  return React.createElement('div', { className:'predict-container' },
    ...
    React.createElement('div', { className:'predict-round-tabs' },
      rounds.map(r => React.createElement('button', {
        key: r,
    ...
    React.createElement('div', { className:'predict-matches' },
      filteredMatches.map(match => React.createElement(PredictCard, {
        key:   match.id,
        match,
        picked: predictions[match.id],
        onPick: (w) => pickWinner(match.id, w)
      }))
    ),
    React.createElement('div', { className:'predict-actions' },
      React.createElement('button', { className:'clear-btn', onClick:clearAll }, '🗑️ Clear All Predictions'),
  ...
  return React.createElement('div', {
    ...
    React.createElement('div', { className:'predict-teams-row' },
      React.createElement('button', {
        ...
        onClick: () => !isPending && onPick(match.home), disabled: isPending
      },
      ...
      React.createElement('div', { className:'predict-middle' },
        React.createElement('button', {
          ...
          onClick: () => !isPending && onPick('draw'), disabled: isPending
        }, picked === 'draw' ? '✓ Draw' : 'Draw')
      ),
      React.createElement('button', {
        className: `predict-team-btn ${picked === match.away ? 'picked' : ''} ${isPending ? 'pending-btn' : ''}`,
        onClick: () => !isPending && onPick(match.away), disabled: isPending
      },
